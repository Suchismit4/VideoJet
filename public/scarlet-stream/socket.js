...
_socket.on('welcome', function(){
	app.id = _socket.id;
	console.log("Connected to the server with socket_id: " + app.id);
});
...
_socket.on('connect-response', function(data) {
	connectionStatus = data.state;
	console.log('Connection response: ' + data.state)
})
_socket.on('user-connected-relay', function(data){
